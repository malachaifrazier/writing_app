class User < ActiveRecord::Base
  ...
  def am_writing?
    if !entries.empty?
      if !desired_interval.nil?
        if track_hours == true
          seconds_written = entries.where(
            created_at: (Time.now.end_of_day - desired_interval.day)..Time.now.end_of_day
          ).sum("duration")
          ...
          times_written = entries.where(
            created_at: (Time.now.midnight - desired_interval.day)..Time.now.midnight
          ).length
  ...
  def total_time_writing_this_week
    if !entries.empty?
      seconds_written = entries.where(
        created_at: (Time.now.end_of_day - 1.week)..Time.now.end_of_day
      ).sum("duration")
